(function ($) {
   ...
   /*---Newsletter Popup activation---*/
	...
	$('.quantity_inner .bt_minus').click(function(event) {
        event.preventDefault();
        let $input = $(this).parent().find('.input-quantity');
        let count = parseInt($input.val()) - 1;
        count = count < 1 ? 1 : count;
        $input.val(count);
    });
    // Прибавляем кол-во по клику
    $('.quantity_inner .bt_plus').click(function(event) {
        event.preventDefault();
        let $input = $(this).parent().find('.input-quantity');
        let count = parseInt($input.val()) + 1;
    ...
    $('.quantity_inner .input-quantity').bind("change keyup input click", function() {
