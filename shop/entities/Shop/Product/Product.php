...
class Product extends ActiveRecord
{
    ...
    public $meta;
    public static function create($brandId, $categoryId, $code, $name, $application_methods, $description, $short_desc, $weight, $quantity, Meta $meta): self

    {
        ...
        $product->application_methods = $application_methods;
    ...
    public function edit($brandId, $code, $name, $application_methods, $description,$short_desc, $weight, Meta $meta): void
    {
        $this->brand_id = $brandId;
        $this->code = $code;
        $this->name = $name;
        $this->application_methods = $application_methods;
    ...
    {
        if ($this->isSale()) {
            throw new \DomainException('Скидка уже действует');
        }
        $this->sale = 1;
    }

    public function notSale(): void
    {
        if (!$this->isSale()) {
            throw new \DomainException('Скидки на этот товар нет');
        }
        $this->sale = 0;
    }
    ...
    {
        if ($this->isNew()) {
            throw new \DomainException('продукт уже в новинках');
        }
        $this->new = 1;
    }
    ...
    {
        if (!$this->isNew()) {
            throw new \DomainException('продукт уже не в новинках');
        }
        $this->new = 0;
    }
