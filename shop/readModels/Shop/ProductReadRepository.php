...
class ProductReadRepository
{
    ...
    {
        return Product::find()->active()->with('mainPhoto', 'category')->orderBy(['id' => SORT_DESC])->limit($limit)->all();
    }
    ...
    {
        return Product::find()
        ->with(['reviews', 'values' => function ($query){$query->joinWith('characteristic')->orderBy(['shop_characteristics.sort' => SORT_ASC]);},])->active()->andWhere(['id' => $id])->one();
    }
    ...
    public static function getWishListCount($userId)
    {
        return  Product::find()
                ->alias('p')->active('p')
                ->joinWith('wishlistItems w', false, 'INNER JOIN')
                ->andWhere(['w.user_id' => $userId])->count();
